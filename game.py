    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if game_state[clicked_row][clicked_col] == ' ':
                    global first_move
                    global current_player

                    if first_move == True or input_is_valid(clicked_col, clicked_row, target_subsection):
                        mark_square(clicked_row, clicked_col, current_player)
                        current_player = switch_player(current_player)
                        target_subsection = subsection(clicked_col, clicked_row)
                        valid_moves = get_valid_moves(game_state, (clicked_row, clicked_col))
                        #print(valid_moves)
                        #agent.generate_next_game_states(game_state, valid_moves)
                        print(game_state)
                        #print(valid_moves)
                        # agent.get_next(get_valid_moves(...)) --> implement this somehow
                        first_move = False
                        over_bool, winner = check_end(game_state)
                        if check_end(game_state)[0] == True:
                            print(f"the game has been won by {winner}")
                    else:
                        print(f'please choose in the proper section ({target_subsection})')
                end_time = time.time()  # End timing
                #print(f"Processing time for MOUSEBUTTONDOWN: {end_time - start_time} seconds")
